...
export function isRateLimitError(error: unknown): boolean {
  ...
  // Check error message (works for both axios and impit)
...
/**
 * Check if an error is an impit-specific error
 * Impit errors have specific message patterns from the rust backend
 */
export function isImpitError(error: unknown): boolean {
  if (!error || typeof error !== 'object') return false;
  ...
  if (typeof err.message === 'string') {
    const message = err.message.toLowerCase();
    // Impit errors contain "impit error" or rust library patterns
    if (message.includes('impit error') ||
        message.includes('reqwest::error') ||
        message.includes('hyper::error') ||
        message.includes('hyper_util')) {
      return true;
    }
  }
...
export function isRetryableError(error: unknown): boolean {
  ...
  // Check for network errors (error codes)
  ...
  // Check error message for transient issues (works for both axios and impit)
  if (typeof err.message === 'string') {
    const message = err.message.toLowerCase();
    ...
    // Impit-specific retryable errors
    if (isImpitError(error)) {
      // Impit timeout errors
      if (message.includes('timeout')) return true;

      // Impit connection errors (from hyper/reqwest)
      if (message.includes('connect') && !message.includes('tls') && !message.includes('ssl')) return true;

      // HTTP/2 protocol errors that may be transient
      if (message.includes('protocol_error') || message.includes('stream')) return true;

      // 5xx server errors in impit messages
