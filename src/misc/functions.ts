...
export async function getMarketSummary(
  ...
  const response = await session.get<{ marketSummaryResponse: { result: MarketSummaryItem[]; error?: { code: string; description: string } } }>(
  ...
  if (response.marketSummaryResponse?.error) {
    throw new Error(`Yahoo Finance API error: ${response.marketSummaryResponse.error.description || response.marketSummaryResponse.error.code}`);
  }
...
export async function getTrending(
  ...
  const response = await session.get<{ finance: { result: TrendingResult[]; error?: { code: string; description: string } } }>(
  ...
  if (response.finance?.error) {
    throw new Error(`Yahoo Finance API error: ${response.finance.error.description || response.finance.error.code}`);
  }

  const result = response.finance?.result?.[0] ?? null;
  if (result === null) {
    throw new Error(`No trending data returned for region ${region}`);
  }
  return result;
}
