  ...
  private processOptionData(data: OptionChainData): OptionChainData {
    ...
    for (const symbol of this._symbols) {
      ...
      if ('options' in symbolData && Array.isArray(symbolData.options)) {
        ...
        // Extract underlying price from quote data (Yahoo options response includes quote)
        const quote = (symbolData as Record<string, unknown>).quote as Record<string, unknown> | undefined;
        let underlyingPrice: number | null = null;
        if (quote && quote.regularMarketPrice !== undefined) {
          // Handle both {raw, fmt} pairs and plain numbers
          const priceVal = quote.regularMarketPrice;
          if (typeof priceVal === 'number') {
            underlyingPrice = priceVal;
          } else if (priceVal && typeof priceVal === 'object' && 'raw' in (priceVal as object)) {
            underlyingPrice = (priceVal as { raw: number }).raw;
          }
        }
