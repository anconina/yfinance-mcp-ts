...
describe('MCP Config', () => {
  ...
  describe('getSessionOptionsFromEnv', () => {
    test('should return default retry config when no env vars set', async () => {
      delete process.env[ENV_VARS.PROXY_LIST];
      delete process.env[ENV_VARS.RETRY_ENABLED];
      delete process.env[ENV_VARS.TIMEOUT];
      delete process.env[ENV_VARS.HTTP_CLIENT];
      delete process.env[ENV_VARS.HTTP3];
      delete process.env[ENV_VARS.IGNORE_TLS_ERRORS];

      const { getSessionOptionsFromEnv: freshFn } = await import('../src/mcp/config');
      const options = freshFn();
      // MCP server always includes default retry config for reliability
  ...
  describe('getMcpSessionOptions', () => {
    test('should cache options', async () => {
      ...
      // Logs: HTTP client, proxy count, retry config on first call only
    ...
    test('should log HTTP client and retry config even without proxies', async () => {
      delete process.env[ENV_VARS.PROXY_LIST];
      const mockStderr = jest.spyOn(console, 'error').mockImplementation();
      ...
      // Should log HTTP client and retry config, but not proxy info
      expect(mockStderr).toHaveBeenCalledTimes(2);
      expect(mockStderr).toHaveBeenCalledWith(expect.stringContaining('HTTP client'));
      expect(mockStderr).toHaveBeenCalledWith(expect.stringContaining('Retry config'));
