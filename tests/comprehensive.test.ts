...
describe('SessionManager - Mocked', () => {
  ...
  describe('Client Access', () => {
    test('should return axios client when using axios backend', () => {
      const session = new SessionManager({ httpClient: 'axios' });
      const client = session.getClient();
      expect(client).toBeDefined();
    });

    test('should return undefined when using impit backend (default)', () => {
      const session = new SessionManager();
      const client = session.getClient();
      // impit is now the default, getClient() returns undefined for impit
